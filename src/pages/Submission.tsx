...
import { ArrowLeft, ArrowRight, Send, Users, Lightbulb, TrendingUp, Target, PieChart, Sparkles, Zap } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import StepFounders from "@/components/submission/StepFounders";
import StepSolution from "@/components/submission/StepSolution";
import StepProgress from "@/components/submission/StepProgress";
import StepMarket from "@/components/submission/StepMarket";
import StepEquity from "@/components/submission/StepEquity";
import StepExpectations from "@/components/submission/StepExpectations";
...
const emptyFounder = (): FounderData => ({});

const OLD_STORAGE_KEY = "aurora_draft_mercado";
...
// key null = usuário ainda não carregado → não persiste (evita gravar rascunho
// numa chave compartilhada sem user_id em máquina de uso comum)
function saveDraft(key: string | null, form: Record<string, string>, founders: FounderData[]) {
  if (!key) return;
...
const steps = [
  ...
  { id: 4, label: "Equity & Governança", icon: PieChart },
  { id: 5, label: "Expectativas", icon: Sparkles },
];
...
const Submission = () => {
  ...
  const STORAGE_KEY = user?.id ? `aurora_draft_mercado_${user.id}` : null;
  ...
  const [founders, setFounders] = useState<FounderData[]>(draft?.founders?.length ? draft.founders : [emptyFounder()]);
  ...
  const founderRequiredKeys: (keyof FounderData)[] = [
    "name", "phone", "email", "gender", "birthdate", "city", "socialMedia",
    "education", "workHistory", "linkedin", "achievements", "projects",
    "title", "founderEquity", "hours", "technical", "inCollege", "founderVideo",
    "techPerson", "howMet",
  ];

  // pitchDeck é opcional na UI (StepSolution) — não entra na validação
  const solutionRequiredKeys = ["solutionName", "shortDescription", "whyIdea", "vertical", "headquarters"];
  const progressRequiredKeys = ["timeWorking", "cashBurn", "revenueProjection", "sellsButFails", "techStack", "reduceCosts", "mvp4weeks"];
  const marketRequiredKeys = ["whyThisIdea", "painPoint", "valueProposition", "whyUs", "competitors", "marketSize", "scalability", "regulatory"];
  const equityRequiredKeys = ["equityBreakdown", "hasCNPJ", "externalInvestment", "thirdPartyDep"];
  const expectationsRequiredKeys = ["whyApply", "expectations"];

  const validateStep = (s: number): boolean => {
    if (isSimplified) {
      // In simplified mode, only solutionName is required
    ...
    const keysMap: Record<number, string[]> = {
      ...
      4: equityRequiredKeys,
      5: expectationsRequiredKeys,
    };
  ...
  const submit = async () => {
    ...
    if (error) {
      ...
      supabase.functions.invoke("send-confirmation-email", {
        body: { userEmail: user.email, userName: founders[0]?.name || "", projectName, submissionType: "mercado" },
      }).catch(() => {});
  ...
  const stepComponents = [
    ...
    <StepEquity form={form} update={update} isSimplified={isSimplified} />,
