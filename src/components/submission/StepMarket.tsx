...
const StepMarket = ({ form, update, isSimplified = false }: Props) => {
  ...
  return (
    <>
      <TextAreaField label="Por que você escolheu desenvolver essa ideia? Você tem experiência na área? Como você sabe que as pessoas precisam do que você está criando?" value={form.whyThisIdea} onChange={(v) => update("whyThisIdea", v)} required={opt} />
      ...
      <TextAreaField label="Responda: Para [Públicos/Clientes] que têm [Problema], nós oferecemos [Solução/Produto]." value={form.valueProposition} onChange={(v) => update("valueProposition", v)} placeholder="Para gestores hospitalares que têm altos índices de absenteísmo, oferecemos..." required={opt} />
