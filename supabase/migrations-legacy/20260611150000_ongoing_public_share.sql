-- ============================================================================
-- Migration: Link público (sem login) da seção Ongoing de uma iniciativa
-- ============================================================================
-- Modelo de segurança:
--   - NENHUMA policy anon é criada nas tabelas de dados (vesting_*).
--   - O acesso público acontece exclusivamente via RPC get_public_ongoing,
--     SECURITY DEFINER, que exige um token UUID não-adivinhável e devolve
--     APENAS os campos necessários (nome do projeto + dados de vesting).
--   - O token vive em ongoing_share_links (1 por submission), gerenciado
--     apenas por admins, e pode ser desabilitado (enabled=false) a qualquer
--     momento — revogação imediata.
--
...
-- Só admins gerenciam (e leem) os links. Anon NÃO tem acesso à tabela.
...
-- 2. RPC pública ------------------------------------------------------------------
-- Devolve os dados do Ongoing para um token válido e habilitado, ou NULL.
-- SECURITY DEFINER: roda como owner, ignorando RLS — por isso o corpo
-- seleciona explicitamente APENAS o necessário (nunca submissions.data,
-- e-mails, etc.).
...
-- Lockdown + grants: anon e authenticated podem EXECUTAR a função (o token é
-- a credencial); ninguém mais.
...
-- ROLLBACK (manual)
-- ============================================================================
-- DROP FUNCTION IF EXISTS public.get_public_ongoing(uuid);
-- DROP TRIGGER IF EXISTS ongoing_share_links_updated_at ON public.ongoing_share_links;
-- DROP TABLE IF EXISTS public.ongoing_share_links;
-- ============================================================================
